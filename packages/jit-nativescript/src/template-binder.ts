...
import { NsNode, NodeType } from '@aurelia/runtime-nativescript';
...
function processTemplateControllers(dom: IDOM, manifestProxy: ParentNodeSymbol, manifest: ElementSymbol): void {
  const manifestNode = manifest.physicalNode as NsNode;
  let current = manifestProxy;
  let currentTemplate: NsNode;
  while (isTemplateControllerOf(current, manifest)) {
    if (current.template === manifest) {
      ...
      if (manifestNode.nodeName === 'Template') {
        ...
        currentTemplate = current.physicalNode = dom.createTemplate() as NsNode;
        currentTemplate.appendChild(manifestNode);
      }
    } else {
      currentTemplate = current.physicalNode = dom.createTemplate() as NsNode;
      currentTemplate.appendChild(current.marker);
    }
...
function processReplacePart(
  ...
  let proxyNode: NsNode;
  let currentTemplate: NsNode;
  if ((manifestProxy.flags & SymbolFlags.hasMarker) > 0) {
    proxyNode = (manifestProxy as SymbolWithMarker).marker as unknown as NsNode;
  } else {
    proxyNode = manifestProxy.physicalNode as NsNode;
  }
  if (proxyNode.nodeName === 'Template') {
    ...
    replacePart.physicalNode = proxyNode as NsNode;
  } else {
    // otherwise wrap the replace in a template
    currentTemplate = replacePart.physicalNode = dom.createTemplate() as NsNode;
    currentTemplate.appendChild(proxyNode);
  }
}
...
export class TemplateBinder {
  ...
  public bind(node: NsNode): PlainElementSymbol {
    ...
    while (i < attributes.length) {
      ...
      if (bindingCommand === null || (bindingCommand.bindingType & BindingType.IgnoreCustomAttr) === 0) {
        ...
        if (attrInfo === null) {
          // map special XML attributes to their corresponding properties
  ...
  private bindChildNodes(
    ...
    let childNode: NsNode | null = node.firstChild;
  ...
  private bindText(
    ...
    const interpolation = this.exprParser.parse(textNode.text, BindingType.Interpolation);
    if (interpolation !== null) {
      const symbol = new TextSymbol(this.dom, textNode, interpolation);
      manifest.childNodes.push(symbol);
      processInterpolationText(symbol);
    }
    let next: NsNode = textNode;
  ...
  private declareTemplateController(
    ...
    const isMultiBindings = attrRawValue.includes(';');
