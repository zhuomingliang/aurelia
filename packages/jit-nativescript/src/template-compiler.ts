...
export class TemplateCompiler implements ITemplateCompiler {
  ...
  public compile(partialDefinition: PartialCustomElementDefinition, context: IContainer): CustomElementDefinition {
    ...
    const template = factory.createTemplate(definition.template) as NsNode;
    ...
    if ((parent.flags & SymbolFlags.hasChildNodes) > 0) {
      ...
      for (let i = 0; i < ii; ++i) {
        ...
        if ((childNode.flags & SymbolFlags.isText) > 0) {
          instructionRows.push([new NsTextBindingInstruction((childNode as TextSymbol).interpolation)]);
  ...
  private compileCustomElement(
    ...
    const instructionRow = this.compileAttributes(symbol, 1) as NsInstructionRow;
  ...
  private compilePlainElement(
    ...
    if (attributes.length > 0) {
      instructionRows.push(attributes as NsInstructionRow);
    }
  ...
  private compileBindings(
    ...
    let bindingInstructions: NsAttributeInstruction[];
    if ((symbol.flags & SymbolFlags.hasBindings) > 0) {
      ...
      bindingInstructions = PLATFORM.emptyArray as typeof PLATFORM.emptyArray & NsAttributeInstruction[];
    }
  ...
  private compileBinding(
    ...
    if (symbol.command === null) {
      ...
      return symbol.command.compile(symbol) as NsAttributeInstruction;
    }
  }
  ...
  private compileAttributes(
    ...
    let attributeInstructions: NsAttributeInstruction[];
    if ((symbol.flags & SymbolFlags.hasAttributes) > 0) {
      ...
      attributeInstructions = PLATFORM.emptyArray as typeof PLATFORM.emptyArray & NsAttributeInstruction[];
    }
  ...
  private compilePlainAttribute(
    ...
    if (symbol.command === null) {
      ...
      if (symbol.expression === null) {
        ...
        return new SetPropertyInstruction(attrRawValue, syntax.target);
      } else {
