...
import { IContainer, PLATFORM, bound, IWindowOrWorkerGlobalScope } from '@aurelia/kernel';
import {
  IDOM,
  QueueTaskOptions,
  TaskQueuePriority,
...
type WindowWithIdleCallback = NodeJS.Global & {
...
const createMicrotaskFlushRequestor = (function () {
  ...
  return function (wnd: WindowWithIdleCallback, flush: () => void) {
    if (called) {
...
const createSetTimeoutFlushRequestor2 = (function () {
  let called = false;
...
const createSetTimeoutFlushRequestor3 = (function () {
  let called = false;
...
const createRequestAnimationFrameFlushRequestor = (function () {
  ...
  return function (window: WindowWithIdleCallback, flush: () => void) {
    ...
    return (function ($window: WindowWithIdleCallback, $flush: () => void) {
      ...
      const request = function () {
        if (timer === void 0) {
          timer = setTimeout(callFlush, /* a frame */16);
        }
        // eslint-disable-next-line no-extra-bind
      }.bind(void 0);
...
const createRequestIdleCallbackFlushRequestor = (function (): any {
  let called = false;
  ...
  return function (window: WindowWithIdleCallback, flush: () => void) {
    ...
    return (function ($window: WindowWithIdleCallback, $flush: () => void) {
      throw new Error('Method not implemented: createRequestIdleCallbackFlushRequestor');
      // let handle = -1;
      ...
      //   ? function () {
      //     throw new Error('Idle callbakc not implemented');
      //     // if (handle > -1) {
...
export class JSDOMScheduler implements IScheduler {
  ...
  public constructor(@IClock clock: IClock, @IDOM dom: NsDOM) {
    ...
    this.flush = [
      ...
      createSetTimeoutFlushRequestor2(wnd, macroTaskTaskQueue.flush.bind(macroTaskTaskQueue)),
      createSetTimeoutFlushRequestor3(wnd, macroTaskTaskQueue.flush.bind(macroTaskTaskQueue)),
    ...
    container.registerResolver(IScheduler, {
      resolve(): IScheduler {
        if (DOM.scheduler === void 0) {
          ...
          const dom = container.get(IDOM) as NsDOM;
